...
export const ahorcado = () => {
  ...
  const printKeyboard = () => {
    for (const letter of keyboard) {
      ...
      letterBtn.disabled = savedGame
        ? savedGame.usedLetters.includes(letter)
        : false;
  ...
  const saveGame = () => {
    const wordSpan = document.querySelectorAll(".word");
    const correctLetters = Array.from(wordSpan)
      .map((span) => span.textContent.trim())
      .filter((letter) => letter !== "_");
    const usedLetters = Array.from(
      keyboardDiv.querySelectorAll("button:disabled")
